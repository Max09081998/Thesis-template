...
\section{Alexa}
	...
	\subsection{Skill di conversazione}
		\subsubsection{Descrizione}
		Le \textit{Skill} consistono in funzionalità personalizzate e aggiuntive per Alexa con l'obiettivo di migliorare l'esperienza d'uso degli utenti. Attraverso le Skill lo sviluppatore può ricevere le richieste rivolte ad Alexa, soddisfarle e restituire una risposta. \\
		Una caratteristica importante è la possibilità di comunicare con servizi Web o applicazioni esterne che forniscono una logica di conversazione aggiuntiva grazie alle \textit{API} presenti nell'\textit{Alexa Skill Kit}.
		\subsubsection{Funzionamento}
		Il principio di funzionamento si articola nei seguenti passi:
		\begin{enumerate}
...
\section{Siri}
	...
	\subsection{Shortcuts}
		\subsubsection{Descrizione}
		Le \textit{Shortcuts} consistono in funzionalità aggiuntive e personalizzate per Siri che migliorano l'esperienza d'uso degli utenti. L'utente può accedere all'applicazione Shortcuts (Comandi se le impostazioni sono in lingua italiana) e al suo interno, per ogni altra applicazione installata nel proprio dispositivo, può visualizzare tutte le frasi messe a disposizione dagli sviluppatori per eseguire operazioni tramite Siri. Può inoltre personalizzare tali frasi per renderle più intuitive e facili da ricordare.
		\subsubsection{Funzionamento}
		Il principio di funzionamento si articola nei seguenti passi:
		...
		\begin{enumerate}
			...
			\item se la ricerca della corrispondenza ha dato esito positivo allora viene selezionato l'intento corretto sulla base del contenuto della stringa;
